...
def main():
    ...
    # Step 2: Read full size of feature_list.py and merge features with the same name and write to features.json
    ...
    # Step 3: Apply feature inclusion filter first (if specified and not empty)
    if args.features and args.features.strip():
        included_features = {name.strip() for name in args.features.split(",") if name.strip()}
        logging.info(f"Including only features: {', '.join(included_features)}")
