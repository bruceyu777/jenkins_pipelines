...
def main():
    ...
    p.add_argument("-n","--nodes",
                   default="node1,node4,node5,node6,node7,node8,node9,node10,node12,node16",
    ...
    # Make sure nodes more than features
    if len(feature_secs) > len(nodes):
        feature_names = ", ".join(feature_secs.keys())
        print(
            f"ERROR: {len(feature_secs)} features to schedule "
            f"but only {len(nodes)} nodes provided:\nNODEs: <{nodes}>\nFEATUREs: <{feature_names}>",
            f"\nPlease provide at least as many nodes as features.",
            file=sys.stderr
        )
        sys.exit(1)
