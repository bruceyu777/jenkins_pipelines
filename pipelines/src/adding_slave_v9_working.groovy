        ...
        stage('Provision Node') {
            ...
            steps {
                script {
                    ...
                    // Extract the secret from the first <argument> element.
        ...
        stage('Install Jenkins Agent on Node') {
            steps {
                script {
                    ...
                    // Build the command to run on the remote node.
...
"""
                    ...
                    // Use sshpass and ssh via sh to execute the remote command.
                    // Ensure sshpass is installed or use SSH key authentication if preferred.
                    def sshCmd = "sshpass -p '${params.NODE_PASSWORD}' ssh -o StrictHostKeyChecking=no ${params.NODE_USER}@${params.NODE_IP} '${installCmd}'"
