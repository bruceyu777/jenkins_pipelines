        ...
        stage('Checkout') {
            steps {
                script {
                    ...
                    currentBuild.displayName = "[FOS GUI] test_gui_csf"
                        ...
                        script {
                            withCredentials([string(credentialsId: 'sudoPassword', variable: 'SUDO_PASS')]) {
                                def status = sh(script: """
                                    ...
                                    echo $SUDO_PASS | sudo -S -u bruce sh -c 'cd ${testFolder} && make docker_test force_build=true name=${keywords} session_id=${env.SESSION_ID} stack=${Config} rerun=${rerun} rp_launch=${rp_launch} reportportal=${reportportal} oriole_config=${oriole_config} rerun=1 browser_type=firefox submit=true headless=true'
                                """, returnStatus: true)
