...
pipeline {
    agent { label 'master' }  // Entire pipeline runs on the built-in (master) node.
    parameters {
        ...
        stage('Create Node') {
            ...
            steps {
                script {
                    ...
                    // Build a Groovy map and convert it to JSON.
                    def payload = [
                        name            : nodeName,
                    ...
                    def pythonCmd = sh(script: 'which python3 || which python', returnStdout: true).trim()
                    echo "Using Python command: ${pythonCmd}"
                    
                    // URL-encode the JSON payload using Python.
                    def encodedJson = sh(
                        script: "${pythonCmd} -c 'import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1]))' \"${jsonBody}\"",
                        returnStdout: true
                    ).trim()
