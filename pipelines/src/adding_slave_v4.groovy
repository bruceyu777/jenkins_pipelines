        ...
        stage('Create Node') {
            ...
            steps {
                script {
                    ...
                    // Build the POST data.
                    def postData = "name=${URLEncoder.encode(nodeName, 'UTF-8')}&type=hudson.slaves.DumbSlave&json=${encodedJson}"
                    echo "POST data: ${postData}"
                    
                    // Use -d to send the POST data.
                    def createResponse = sh(
                        script: "curl -s -u ${params.JENKINS_ADMIN_USER}:${params.JENKINS_API_TOKEN} -H 'Jenkins-Crumb:${crumb}' -X POST -d \"${postData}\" '${params.JENKINS_URL}/computer/doCreateItem'",
                        returnStdout: true
                    ).trim()
