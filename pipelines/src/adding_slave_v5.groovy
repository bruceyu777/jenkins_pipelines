            ...
            steps {
                script {
                    ...
                    def payload = [
                        ...
                        nodeProperties  : [], // empty list instead of map
                        launcher        : [
                    ...
                    // Use --data-urlencode for each parameter to avoid shell quoting issues.
                    def createResponse = sh(
                        script: "curl -s -u ${params.JENKINS_ADMIN_USER}:${params.JENKINS_API_TOKEN} -H 'Jenkins-Crumb:${crumb}' -X POST --data-urlencode \"name=${nodeName}\" --data-urlencode \"type=hudson.slaves.DumbSlave\" --data-urlencode \"json=${jsonBody}\" '${params.JENKINS_URL}/computer/doCreateItem'",
                        returnStdout: true
                    ).trim()
