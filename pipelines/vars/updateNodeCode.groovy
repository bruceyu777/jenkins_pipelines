...
def call() {
    ...
    pipeline {
        ...
        post {
            always {
                script {
                    echo "Pipeline completed. Sending email."
                    ...
                    try {
                        ...
                        echo "Failed to read node info: ${e.getMessage()}"
                    }
                    ...
                    sendFosqaEmail(
                        to     : sendTo,
                        subject: "${status}: Node Update ${env.BUILD_DISPLAY_NAME}",
                        body   : """
                        <p>${status == 'SUCCESS' ? '🎉 Good news!' : '⚠️ Note:'} Node update job <b>${env.BUILD_DISPLAY_NAME}</b> completed at ${new Date()} with status: <b>${status}</b></p>
